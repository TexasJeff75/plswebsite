/*
  # Create Facility Merge Function

  1. New Functions
    - `merge_facilities(source_facility_id uuid, target_facility_id uuid)`
      - Merges all related records from source facility to target facility
      - Updates foreign keys in all related tables:
        * activity_log
        * documents
        * equipment
        * facility_contacts
        ...
        * trained_personnel
        * training_info
      - Deletes the source facility after merge
      - Returns success status

  2. Security
    - Function runs with SECURITY DEFINER to bypass RLS
    - Only callable by authenticated users with Editor or Admin role
*/
...
BEGIN
  ...
  IF v_user_role NOT IN ('Editor', 'Admin') THEN
    RAISE EXCEPTION 'Only Editors and Admins can merge facilities';
  END IF;
...
COMMENT ON FUNCTION merge_facilities IS 'Merges all related records from source facility to target facility, then deletes the source facility. Only accessible to Editors and Admins.';
