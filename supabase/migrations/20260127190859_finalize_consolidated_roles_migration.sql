/*
  # Finalize Consolidated Roles Migration

  1. Updates all RLS policies to use new consolidated role names
  2. Removes user_type column dependency
  3. Adds role validation constraint

  Important: Role data migration already completed via execute_sql
*/
...
CREATE POLICY "Admins can insert organizations"
  ...
  WITH CHECK (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'Proximity Admin'));

CREATE POLICY "Admins can update organizations"
  ON organizations FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role = 'Proximity Admin'));
...
CREATE POLICY "Staff can read all facilities"
  ...
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Account Manager', 'Technical Consultant', 'Compliance Specialist')));
...
CREATE POLICY "Staff can insert facilities"
  ...
  WITH CHECK (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Account Manager', 'Technical Consultant')));

CREATE POLICY "Staff can update facilities"
  ON facilities FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Account Manager', 'Technical Consultant', 'Customer Admin')));

CREATE POLICY "Admins can delete facilities"
  ON facilities FOR DELETE TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Customer Admin')));
...
CREATE POLICY "Staff can modify documents"
  ...
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Account Manager', 'Technical Consultant', 'Compliance Specialist', 'Customer Admin')));
...
CREATE POLICY "Staff can modify regulatory_info"
  ON regulatory_info FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Compliance Specialist', 'Customer Admin')));
...
CREATE POLICY "Staff can modify personnel_info"
  ON personnel_info FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Technical Consultant', 'Customer Admin')));
...
CREATE POLICY "Staff can modify trained_personnel"
  ON trained_personnel FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Technical Consultant', 'Customer Admin')));
...
CREATE POLICY "Staff can modify integration_info"
  ON integration_info FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Technical Consultant', 'Customer Admin')));
...
CREATE POLICY "Staff can modify interface_status"
  ON interface_status FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Technical Consultant', 'Customer Admin')));
...
CREATE POLICY "Staff can modify facility_readiness_info"
  ON facility_readiness_info FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Technical Consultant', 'Customer Admin')));
...
CREATE POLICY "Staff can modify training_info"
  ON training_info FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM user_roles WHERE user_id = auth.uid() AND role IN ('Proximity Admin', 'Proximity Staff', 'Technical Consultant', 'Customer Admin')));

-- Drop user_type column now that all policies are updated
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'valid_user_role' AND table_name = 'user_roles'
  ) THEN
    ALTER TABLE user_roles ADD CONSTRAINT valid_user_role
    CHECK (role IN ('Proximity Admin', 'Proximity Staff', 'Account Manager', 'Technical Consultant', 'Compliance Specialist', 'Customer Admin', 'Customer Viewer'));
  END IF;
END $$;