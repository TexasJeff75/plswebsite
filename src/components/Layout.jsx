...
export default function Layout() {
  ...
  if (profile?.role === 'Super Admin' || profile?.role === 'Proximity Admin') {
    navItems.push({
      path: '/invoicing',
      label: 'Invoicing',
      icon: (
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {navItems.map((item, index) => {
            if (item.type === 'section') {
              return (
                <div key={`section-${index}`} className={`${index > 0 ? 'mt-6' : 'mt-3'} mb-2 ${sidebarCollapsed ? 'px-3' : 'px-3'}`}>
                  {!sidebarCollapsed && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-orange-100 text-sm">
                  You are viewing as this user. Original admin: {originalAdmin?.profile?.display_name || originalAdmin?.user?.email}
                </p>
              </div>
            </div>
            <button
              onClick={stopImpersonation}
              className="flex items-center gap-2 px-4 py-2 bg-white text-orange-600 hover:bg-orange-50 rounded-lg font-medium transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
